...
- **Embedded video**: when a launch demo or product video shows what prose cannot. For YouTube, embed the video inline by default when the publishing target supports it. Do not collapse the main video behind a details element, make an external YouTube link the primary action, or place custom overlay UI on top of the native player.
